...
CREATE INDEX IF NOT EXISTS finance_payments_provider_code_idx
  ON dciecms_test.finance_payments(provider_code, status, created_at)
  WHERE provider_code IS NOT NULL;

CREATE TABLE IF NOT EXISTS dciecms_test.payment_callback_events (
...
CREATE INDEX IF NOT EXISTS payment_callback_events_payment_received_idx
  ON dciecms_test.payment_callback_events(payment_id, received_at);

CREATE INDEX IF NOT EXISTS payment_callback_events_processing_received_idx
  ON dciecms_test.payment_callback_events(processing_status, received_at);

REVOKE DELETE ON dciecms_test.payment_callback_events FROM PUBLIC;
