...
CREATE OR REPLACE FUNCTION documents.enforce_document_byte_immutability()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS documents_bytes_immutable_trg ON documents.documents;
CREATE TRIGGER documents_bytes_immutable_trg
BEFORE UPDATE ON documents.documents
FOR EACH ROW
EXECUTE FUNCTION documents.enforce_document_byte_immutability();
